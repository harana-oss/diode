...
trait BaseModelR[M, S] extends ModelR[M, S] {
  ...
  override def zoom[T](get: S => T)(implicit feq: FastEq[? >: T]) =
    new ZoomModelR[M, T](root, get compose this.eval)

  override def zip[SS](that: ModelR[M, SS])(implicit feqS: FastEq[? >: S], feqSS: FastEq[? >: SS]) =
